...
## F117 — OSS 公開準備（CI / PR運用 / バージョン・タグ管理）

| # | 領域 | 事実 | 摩擦度 |
|---|---|---|---|
| F117 | 出発点は「何も無い」 | `.github/`なし、LICENSEなし、CHANGELOGなし、ルートREADMEなし、CONTRIBUTINGなし、CIなし。4パッケージ全てのmix.exsが`version: "0.1.0"`のまま（コミット履歴はv0.1〜v0.5を名乗っている）。つまり外部PRが来ても人力レビューしか手段が無い状態だった。 | - |
| F117 | ツールチェーンの実測（推測で書くと初回CIが確実に落ちる箇所） | ローカルの実効ツールチェーンはmiseではなく**Homebrew**（`which elixir`→`/opt/homebrew/bin/elixir`、erlang 29.0.4 / elixir 1.20.2）。miseには`elixir 1.20.2-otp-28`/`erlang 28.5.0.4`という古い組がインストール済みだが非アクティブで、`mise current`は空——`mise ls`だけ見て`.tool-versions`を書くと実環境と食い違う罠。さらに**ローカルのelixirはotp-28ビルドがOTP 29上で動いている**状態（動くが本来の組ではない）。`.tool-versions`には正しい組（`elixir 1.20.2-otp-29`）を書いた。 | 中 |
| F117 | プリビルドの実在確認（GitHub Actionsで一番よく落ちる原因） | `builds.hex.pm/builds/elixir/builds.txt`に`v1.20.2-otp-29`が存在、`builds.hex.pm/builds/otp/ubuntu-24.04/builds.txt`に`OTP-29.0.4`が存在——両方をcurlで実測してから`setup-beam`を選択した（存在しなければ`hexpm/elixir:...`コンテナ実行にフォールバックする必要があった）。なおOTPビルドのURLは`/builds/otp/<os>/builds.txt`で、アーキテクチャを挟む`/builds/otp/x86_64/<os>/`は404——最初に試して外した。 | 中 |
| F117 | `setup-beam`の`version-file`は`version-type: strict`が**必須** | READMEを直接読んで確認（"if you're using a version file, option `version-type` is checked to be `strict`, and will make the action exit with error otherwise"）。キー対応も実測確認: `.tool-versions`の`erlang`→`otp-version`、`elixir`→`elixir-version`。記憶で書けば初回runがstep 2で落ちていた。 | 低 |
| F117 | compile→formatの順序には根拠がある | `.formatter.exs`を4パッケージ分grepしたところ3つが`plugins: [Spark.Formatter]`（kumi_adminは加えて`Phoenix.LiveView.HTMLFormatter`）を読む。プラグインはコンパイル済みでないと`mix format`がロードできないため、コールドキャッシュのCIでformatを先に置くと意味不明なエラーで落ちる。ワークフローにこの理由をコメントで残した。 | 低 |
| F117 | Postgresサービスのポートを5434のままにした | `kumi/config/test.exs`が既に`port: 5434`を見ているので、サービスコンテナを`5434:5432`で公開すればリポジトリ内のconfigにCI専用分岐が一切要らない。「CIのためにコードを曲げる」ことを避けた設計判断。Postgresを必要とするのは`kumi`だけ（他3つは純粋な単体テスト）だが、matrixの`services`は条件分岐できないため全ジョブで起動——数秒のコストを受け入れてmatrixを二分割しない方を選んだ。 | - |
| F117 | 初回CIが一発green（修正ラウンドゼロ） | run 33091298788、4ジョブ全て success、約6分。**テスト数がローカルと完全一致**（kumi 168 / kumi_admin 74 / kumi_new 52 / kumi_storage 30 = 324）——ログから`Result: N passed`をgrepして確認したので「テストがスキップされて緑」ではない。今セッション5回踏んだ「suite green ≠ 実際に動く」を回避するため、事前に4パッケージでCIと同一の3コマンドを実測してから書いた効果。 | - |
| F117 | push時にremoteと衝突——**ユーザがweb UIで`SECURITY.md`を追加していた** | `git push`がnon-fast-forwardで拒否。fetchすると`a423558 Create SECURITY.md for security policy`（GitHubのweb editorのデフォルトコミットメッセージ）がremoteにあり、こちらもローカルで`SECURITY.md`を新規作成していたためadd/add conflict。中身を`git show origin/main:SECURITY.md`で確認したところ**GitHubテンプレートの未編集版**（`5.1.x` `:white_check_mark:`等のプレースホルダそのまま）だったので、Kumi固有の内容（kumi.applyの4ゲート、Safetyのfail-closed、storageのパス処理、adminの借り物認証を「調査推奨箇所」として明示）を採用。**force pushは使わず**rebase＋`checkout --theirs`で解決し、ユーザのコミットは履歴に残した。教訓：長時間の作業中にユーザがremoteを触っている可能性があるので、pushの前提は「fetchしてから判断」。 | 中 |
| F117 | lockstepバージョニングを選んだ理由 | 4パッケージ共通の1バージョン＋単一`vX.Y.Z`タグ。根拠：`kumi_admin`/`kumi_storage`は開発中はpath依存で`kumi`を参照し、Hex公開時にversion依存へ置換する——版が独立して動くと毎リリース互換マトリクスが必要になるが、lockstepなら`{:kumi, "~> X.Y"}`で機械的に済む。コストは「無変更のパッケージも版が上がる」ことで、これはRELEASING.mdに正直に書いた。per-packageタグ（`kumi/vX.Y.Z`）は実際に版が乖離する必要が出た時点で切り替える方針とし、先回りして作らない（YAGNI）。 | - |
| F117 | 意図的に作らなかったもの | (1) **LICENSE**——名前とライセンスは人間の未決定事項。Hex `package/0`メタデータも同時にブロックされるため、プレースホルダを置かず「公開前チェックリスト」に載せた（不完全なHexメタデータは`mix hex.build`を分かりにくく壊す）。(2) **CODE_OF_CONDUCT.md**——本物の連絡先が必要で、未記入プレースホルダ入りのCoCは無い方がマシなのでチェックリスト送り。(3) **カバレッジゲート**——excoverallsの導入はスコープクリープ。既に効いているゲート（format / warnings-as-errors / test）で十分と判断。 | - |
| F117 | ブランチ保護はファイルでは表現できない | `main`への必須ステータスチェック（4ジョブ名）、squash-merge限定、private vulnerability reporting有効化、初回コントリビュータのworkflow承認必須——全てGitHub側の設定。RELEASING.mdに`gh api`コマンドとチェックリストとして書き残した（リポジトリ内の何もこれを強制できないため、忘れる前提で明文化）。 | - |
| F117 | CONTRIBUTING.mdの自己完結性 | 公開後もblueprintとfriction log（＝このファイル）は私物・日本語のまま残す方針なので、CONTRIBUTINGはそれらへの参照をゼロにした。代わりに、外部の人が知る必要のある事実（planパイプラインのrename→safety順序、ゼロdiffアサーションが回帰ゲートであること、`MIX_ENV=test mix ash.codegen`の罠、「空虚に真なテスト」の実例）は本文に直接書き写した。 | - |

## F118 — レビュー指摘25件の修正（Blocker 1 / High 5 / Medium 10 / Low 9）

| # | 領域 | 事実 | 摩擦度 |
|---|---|---|---|
| F118 | 分割方針：ファイル所有権で切る | 25件を7グループに割り、各エージェントに**編集可能ファイルの明示リスト**を渡した。`safety.ex`/`sql.ex`/`fix_hint.ex` を H4/H5/M1/L3 と H3/M2 の双方が触るため、この2グループだけ直列にした（並列だと後発の Edit が「ファイルが変わった」で失敗する）。結果、6並列＋1直列＋CI は自分。所有権を明示しなかった場合に起きるはずだった衝突は0件で、各エージェントの報告も「他の並行エージェントのファイルは触っていない」を`git status`で自己確認していた。**同一パッケージ内の並列作業は、ディレクトリ単位ではなくファイル単位で線を引けば成立する**という実証。 | - |
| F118 | 副作用：並行編集中は共有ハーネスが不安定になる | `mix test`/`mix format --check-formatted`/`mix compile --warnings-as-errors` は全エージェントが共有するので、他が編集中のファイルのせいで自分の検証が落ちる。実際に複数エージェントが「一時的に落ちたが後で直った」と報告。さらに`test/mix/tasks/kumi.report_test.exs`が`Jason.DecodeError: unexpected byte at position 0: 0x43 ("C")`で落ちた——サブプロセスの`mix kumi.report --json`が古いビルドを検出して`Compiling 2 files (.ex)`をJSONの前にstdoutへ吐いたため。**これは並行作業の産物だが、潜在的な脆さとしては本物**（`mix kumi.report --json \| jq`はビルドが古いと壊れる）。25件外なので記録のみ。 | 中 |
| F118 | **親セッションが自分で fail-open を作り込んだ（今回いちばんの学び）** | H1（`mix kumi.expand`が全部出さない）の修正機構について、委譲プロンプトに「`@before_compile`ではSparkのDSL状態が未確定だから`@after_verify`を使え」と書いた。**この前提が誤り**で、実測すると`Module.get_attribute(mod, :spark_dsl_config)`は`@before_compile`で読める。ただし条件付きで、`Kumi.Resource.__using__`は`@before_compile Kumi.Resource`を`use Ash.Resource`より**先に**登録しているためSparkより先に走り、configがnilだった。そこへ自分が`\|\| %{}`のフォールバックを置いた結果、**チェックが黙って no-op になった**（違反モジュールがexit 0でコンパイル通過）。今セッションで散々「fail closedにせよ」と直してきた側が、同じ穴を自分の手で開けた。修正：(1)`@before_compile`の登録を`use Ash.Resource`の後ろへ移動、(2)`\|\| %{}`を撤去し**configがnilなら「Sparkの順序が変わった＝チェックを直せ」とraise**。教訓は「フォールバックのデフォルト値は、それ自体が『異常を正常に見せる』経路になり得る」——特に検査系コードでは`\|\| []`/`\|\| %{}`が最も危険。 | 高 |
| F118 | 上記の副産物：エラーUXが根本的に良くなった | `@after_verify`は`Module.ParallelChecker`の別プロセスからraiseするので、ユーザーには`** (EXIT from #PID<0.94.0>) an exception was raised:`という生ダンプで届く（メッセージ自体は読めるが、Phoenix/Ash初心者に見せる画面ではない）。`@before_compile`へ移すと`** (CompileError) /path/to/user_file.exs:1: Kumi.Resource: ...`となり**ユーザー自身のファイルを指す**。`EXIT from #PID`の出現は0行、exit codeは1のまま。「Ashを誰でも使えるように」がこのプロジェクトの目的なので、この差は機能的には同等でも製品としては大きい。さらに生のDSL configを読むため`public? false`のリレーションも見える——エージェントが「`public_relationships/1`だとprivateなリレーションを取りこぼす」と正直に申告していた穴が自動的に閉じた。テスト側もTask+trap_exitの迂回が不要になり素の`rescue`に簡略化。 | - |
| F118 | 設計判断：H3の新オペは「REVIEW＋`:unsupported`」で fail closed が自動的に成立する | PK/FK/index の変更を検出する3オペ（`change_primary_key`/`change_fk`/`change_index`）を追加する際、分類を**REVIEW**（データ削除ではないが純粋な追加でもない＝既存ルールの「制約の締め付け・推測」バケツ）、`SQL.render/1`は**`:unsupported`**（DROP+CREATEの2文が必要で、このモジュールの契約「1オペ=1文」に収まらない）とした。この2つの組み合わせで`Kumi.Apply`の第3ゲートが自動的に弾くので、`@safe_op_tags`を触る必要がなく、「SQLテキストの単一ホーム」原則も崩れない。実測：`Apply.preview`が`exec=0 skip=3`。**新機能を追加するとき、既存の不変条件が勝手に効く形に落とすのが一番安全**という例。 | - |
| F118 | 危険だったのは「網羅matchでcatch-allが無い」箇所 | 新オペ追加で最も壊れやすいのは、opをパターンマッチしている全箇所。エージェントが洗い出した結果、`lib/kumi/plan/format.ex`と`lib/kumi/report/format.ex`が**catch-allなしの網羅match**で、節を足さなければ`FunctionClauseError`でplan全体が落ちていた。逆に`Kumi.Probe`（`probe(_repo,_op), do: []`）、`Kumi.Plan.Rename`（汎用4要素節）、`Kumi.Report.Json`（`Format.describe/1`へ委譲）は無改造で通った。**「盲点を埋める修正が、埋めた先でクラッシュを作る」のが最悪なので、新オペ導入時のgrepは必須手順**。 | 中 |
| F118 | エージェントのDB操作の正直な申告（評価できる挙動） | H3担当が`custom_indexes`のfixture列を最初`:stage`にしてしまい、`ash.setup`経由で`kumi_test_deals_stage_index`を実DBに作成→既存の`Kumi.ApplyTest`（「stage列をdropしたらadd_columnが1件だけ」）が`add_index`の連鎖で落ちた。**既存テストを書き換えず**列を`:amount`へ変更し、自分が作ったindexを`DROP INDEX IF EXISTS`で撤去。さらに`schema_migrations`の孤児行を消そうとしたが権限分類器に阻まれ、迂回せず「無害だが残っている」と申告した。親セッションで検証：孤児行`20260827175620`は実在するがファイルは無く、`MIX_ENV=test mix ash.codegen --check`が「up to date (no pending migrations)」を返すので**無害を実証**（CIは毎回DBを新規作成するので影響なし）。ユーザーのDBを勝手にdropしないため、清掃は提案のみに留めた。 | - |
| F118 | Medium/Lowで判明した「指示の前提が実装と違う」ケース | (1) M3で「Forbiddenならforbidden、それ以外はraise」と指示したが、担当が Ash のソースを読んで**filter policyで拒否された`Ash.get`は`Ash.Error.Forbidden`をraiseしない**（`Ash.Error.Invalid`が`Ash.Error.Query.NotFound`を包む＝素の404と同一）ことを確認し、リテラルに従えば「古いリンクを踏むたびクラッシュ」になると指摘。ForbiddenとそのNotFound形状の両方を既存の曖昧な「No access or no record.」に寄せ、それ以外をraiseする形に変更した。**指示より実装の事実を優先した判断で、これは正しい**。(2) L8で「空白のみの検索語は素通し」と前提を書いたが、`Search.apply/3`のガードは`term in [nil, ""]`なので`"   "`はリテラル検索に落ちる。担当は挙動を変えずテストで文書化し製品判断として残した。 | 中 |
| F118 | 実測テスト数 | kumi 168→**219**、kumi_admin 74→**113**、kumi_storage 30→**34**、kumi_new 52→52（無改造）。合計 324→**418**（+94）。4パッケージすべて`mix compile --warnings-as-errors`・`mix format --check-formatted`クリーン、ゼロdiff回帰ゲート通過。CIは`erlef/setup-beam`をcommit SHA（`54075bcc`=v1.24.1）に固定し`timeout-minutes: 20`を追加。 | - |
| F118 | 検証方針：修正前の自分の実測値を残しておくと後が楽 | レビュー段階で親セッションが取った probe スクリプト（`xss_probe.exs`/`diff_probe.exs`/`core_probe.exs`/`sql_probe.exs`）を scratchpad に残していたので、修正後に**同一スクリプトを回すだけ**で前後比較が取れた。エージェント自身の「before」は`git show HEAD:...`からの再構成だったり「briefの記載を採用」だったりして証拠として弱いので、**発見者が修正前に実測値を保存しておく**のが検証コストを下げる最良の手。 | - |

## F119 — オンボーディング導線の穴（ルートREADME・パッケージREADME欠落）

| # | 領域 | 事実 | 摩擦度 |
|---|---|---|---|
| F119 | **ルートREADMEの最初のコマンドが一度も実行されていなかった** | `README.md`の"Quick look"冒頭が`mix kumi.new my_crm`だったが、これは`--kumi-path is required until kumi ships to Hex`で必ず失敗する。`mix archive.build && mix archive.install`の手順も欠落。`kumi/README.md`と`kumi_new/README.md`には正しい実測済み手順があり、**ルートREADMEだけが実測スニペット文化の外にあった**。新規の人が最初にコピペするコマンドが失敗する状態が、v0.4のCI整備を通り越して残っていた。教訓：「全スニペット実測」を謳うなら、対象リストにルートREADMEを明示的に含める（配下パッケージのREADMEが正しいことは、上位のREADMEの正しさを保証しない）。 | 高 |
| F119 | `--kumi-path`は**正規パスでないと**生成プロジェクトがMixに拒否される | 検証で`--kumi-path Kumi`（Kumiは実リポジトリへのsymlink）を実行したところ、生成後の`mix format`段階で`the dependency kumi in mix.exs is overriding a child dependency: > In mix.exs: {:kumi, path: ".../readme_check/Kumi/kumi"} > In kumi_admin/mix.exs: {:kumi, path: "../kumi"}`→`** (Mix) Can't continue due to errors on dependencies`。原因は`KumiNew.Args.fetch_kumi_path/1`が`Path.expand`しかせずsymlinkを解決しないため、生成mix.exsに書かれる絶対パスとkumi_admin側の`../kumi`の解決結果が**文字列として一致しない**こと。正規の絶対パスで再実行するとexit 0・`==> my_crm is ready.`まで到達し、`--no-setup`も正常だった。macOSの`/tmp`→`/private/tmp`でも同じ罠を踏むはずなので、READMEに症状のエラーメッセージ込みで注記した。**根本修正（`--kumi-path`のrealpath解決）は未実施**——Hex公開でこのフラグ自体が消える予定なので、消える機能に実装を足さない判断。 | 中 |
| F119 | `kumi_admin`と`kumi_storage`にREADMEが無かった | 看板であるadmin UIの説明が`kumi/README.md`の1段落と`guides/mini-crm.md`の中だけにあり、GitHubで`kumi_admin/`を開いた人には何も表示されない状態。両方を新規作成し、**既に検証済みの一次情報からのみ**構成した（router moduledoc、installタスクのDocs module、`Validation`の既定値、`Backend`のbehaviour moduledoc）。新規に文章を発明せず既存の正確な記述を移植する方針で、実行不能なスニペットの再生産を回避。adminのREADMEには新規ユーザーが必ず踏む2点（認証を持たないこと、管理対象リソースは`:id`単一主キー必須）を独立セクションとして立てた。 | - |
| F119 | 「使いやすさ」の残る障壁は実装ではなく未決定事項 | 中身（実測済みガイド4本、既存アプリ向けinstallタスク、親切なエラーメッセージ、CI 4/4 green）は揃っているが、LICENSE未定でREADME自身が"please don't build on this yet"と書いており、Hex未公開のため全導入経路がローカルcheckout前提。**入口を塞いでいるのはコードではなく人間の決定**（RELEASING.mdの公開前チェックリスト）。ドキュメント側でこれ以上詰められる余地は小さい。 | - |
