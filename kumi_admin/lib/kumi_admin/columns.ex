defmodule KumiAdmin.Columns do
  ...
  non-sensitive attributes, in declaration order (which puts `id` first
  for every `uuid_primary_key`-based resource). No config, no priority
  scoring: `KumiAdmin.Attributes.visible/1` already gives us the order a
